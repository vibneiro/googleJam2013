...
/**
 ...
 Problem D
 Small inputs
 Comment: Poor time complexity - O(n!) permutations
 */
public class Treasure {
    ...
    private void solve(Scanner sc, PrintWriter pw) {
        ...
        for(int i = 0; i < numOfChests; i++) {
            ...
            System.out.println("keysInside =  " + chest.keyTypesInside.toString());
    ...
    private String traverse(List<Integer> listOfChests) {
        ...
        while(keyTypesIHaveCopy.size() > 0 && chestCounter < numOfChests) {
            ...
            if(keyTypesIHaveCopy.size() == 1 && chest.isEmpty() && openChests == 0) {
                chestCounter = incrementChestNumber(chestCounter, numOfChests);
                continue;
            }

            if (keyTypesIHaveCopy.contains(chest.keyTypeToOpen)) {
                System.out.println("keyTypesIHave = " + keyTypesIHaveCopy.toString() + " chest.keyTypeToOpen = " + chest.keyTypeToOpen);
                keyTypesIHaveCopy.remove(chest.keyTypeToOpen);
                keyTypesIHaveCopy.addAll(chests.get(chestNumber).keyTypesInside);
                System.out.println("And now keyTypesIHave = " + keyTypesIHaveCopy.toString());
    ...
    public static void main(String[] args) throws Exception {
        ...
        Scanner sc = new Scanner(new FileReader("D-small-attempt0.in.txt"));
